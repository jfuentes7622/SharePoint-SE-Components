...
export interface ISharePointDynamicFormWebPartProps {
  ...
  filterDesignerOperator?: 'eq' | 'ne' | 'gt' | 'ge' | 'lt' | 'le' | 'contains' | 'notcontains' | 'startswith' | 'endswith';
...
export default class SharePointDynamicFormWebPart extends BaseClientSideWebPart<ISharePointDynamicFormWebPartProps> {
  ...
  private _filterFields: IDropdownOption[] = [];
  ...
  private refreshFullWidthLayout(): void {
  ...
  private async loadLists(): Promise<void> {
    try {
      ...
      console.warn('[ReportFormsWebPart] loadLists: Failed to load site lists: ', error);
    }
  }
  ...
  private async loadListFields(listName: string): Promise<void> {
    if (!listName) {
      ...
      this._filterFields = [{ key: '', text: strings.PropDynamicTargetFieldNone }];
    ...
    try {
      var webUrl = this.context.pageContext.web.absoluteUrl.replace(/\/$/, '');
      var response = await this.getWithAcceptFallback(
        webUrl + "/_api/web/lists/getByTitle('" + escapeODataText(listName) + "')/fields?$select=InternalName,Title,TypeAsString,Hidden,ReadOnlyField,Sealed,LookupList,Choices&$filter=Hidden eq false"
      );
      var data = await response.json();
      ...
      var toOption = function(field: any) {
      ...
      var filterOptions = fields.map(toOption).sort(function(a: IDropdownOption, b: IDropdownOption) {
        return String(a.text).localeCompare(String(b.text));
      });
      var options = fields
        .filter(function(field: any) { return (field.ReadOnlyField !== true && field.Sealed !== true) || field.InternalName === 'Attachments'; })
        .map(toOption)
        .sort(function(a: IDropdownOption, b: IDropdownOption) { return String(a.text).localeCompare(String(b.text)); });

      this._listFields = options.length > 0 ? options : [{ key: '', text: strings.PropDynamicTargetFieldNone }];
      this._filterFields = filterOptions.length > 0 ? filterOptions : [{ key: '', text: strings.PropDynamicTargetFieldNone }];
      ...
      this._fieldLookupListByInternalName = {};
      this.context.propertyPane.refresh();
      console.warn('[ReportFormsWebPart] loadListFields: Failed to load fields for list "' + listName + '": ', error);
    }
  }
  ...
  private async loadLookupListItems(listId: string): Promise<IDropdownOption[]> {
    try {
      ...
      console.warn('[ReportFormsWebPart] loadLookupListItems: Failed to load items for list "' + listId + '": ', error);
      return [];
    }
  }
  ...
  private pushDynamicDiagnostic(message: string): void {
    ...
    console.log('[ReportFormsWebPart] ' + entry);
  ...
  private handleAddFilterDesignerCondition(): void {
    try {
      ...
      var condOperator = (this.properties.filterDesignerOperator || 'eq') as 'eq' | 'ne' | 'gt' | 'ge' | 'lt' | 'le' | 'contains' | 'notcontains' | 'startswith' | 'endswith';
      ...
      if (valueSource === 'dynamic') {
        ...
        if (valueSource === 'expression') {
          ...
          if (conditionValue.length === 0) {
            this._filterDesignerMessage = 'Select at least one field value.';
            this.context.propertyPane.refresh();
            return;
          }
  ...
  private handleLoadSelectedFilterCondition(): void {
    try {
      ...
      this.properties.filterDesignerValue = valueSource !== 'dynamic'
        ? (Array.isArray(selectedValue) ? JSON.stringify(selectedValue) : String(selectedValue === undefined || selectedValue === null ? '' : selectedValue)) : '';
  ...
  protected getPropertyPaneConfiguration(): IPropertyPaneConfiguration {
    ...
    var dynamicTargetFieldOptions = this._dynamicTargetLookupFields.length > 0
      ? this._dynamicTargetLookupFields
      : [{ key: '', text: strings.PropDynamicTargetFieldNone }];
    var filterFieldOptions = this._filterFields.length > 0
      ? this._filterFields
      : [{ key: '', text: strings.PropDynamicTargetFieldNone }];
    ...
    return {
            ...
            },
            {
              ...
              groupFields: [
                PropertyPaneLabel('propertyPanePageGuide1', {
                  text: 'Configure the report list, record selection, layout, and permissions. Report Forms is always read-only.'
                })
              ]
            },
            {
          ...
          groups: [
            {
              groupName: '',
              groupFields: [
                this.createPropertyPanePageTitle('filterDesignerPageTitle', 'Filters')
              ]
            },
            {
              groupName: '',
              groupFields: [
                PropertyPaneLabel('filterDesignerHelp', {
                  text: 'Choose report-list columns and combine conditions with AND/OR to select the record displayed by this Report Form. These conditions are combined with linked-field filtering. An explicit item ID takes precedence.'
                }),
                ...
                PropertyPaneDropdown('filterDesignerOperator', {
                  ...
                  options: [
                    ...
                    { key: 'notcontains', text: 'Does not contain (text)' },
      ...
      ].filter((_page, pageIndex) => pageIndex === 0 || pageIndex === 2)
    };
  }
