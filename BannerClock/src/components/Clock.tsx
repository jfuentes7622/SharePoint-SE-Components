...
import { IClockProps, IClockState } from '../Interfaces/IClock';
import {IdateTimeFormatOptions} from '../Interfaces/DateTimeFormatOptions';

export class Clock extends React.Component<IClockProps, IClockState> {
 // private _timerID: ReturnType<typeof setInterval>;
  private _timerID: number;
  private _formatOptions: IdateTimeFormatOptions = {hour: '2-digit', minute: '2-digit', second: '2-digit', hourCycle: 'h23', weekday: undefined, timeZone: undefined};
  private _dateString : string;
  private _isValidTimezone : boolean = true;
  ...
  constructor(props: IClockProps) {
    ...
    //add the timezone to the format options
    this._formatOptions.timeZone = props.timeZone;

    //check if the 12 hour format was required by props
    if(this.props.hour12) {
      this._formatOptions.hourCycle = 'h12';
    }
    else{
      this._formatOptions.hourCycle = 'h23';
    }

    //check if the display day option was selected
    if(this.props.displayDay) {
      this._formatOptions.weekday = 'short';
    }
    else{
      this._formatOptions.weekday = undefined;
    }

    //format the current time, to check for issues with the timezone supplied
    try{
      this._dateString = Intl.DateTimeFormat('default', this._formatOptions).format(new Date(Date.now()));
    }
    catch(error){
      if(error instanceof RangeError){
        //not a valid timezone
        this._isValidTimezone = false;
      }
      else {
  ...
  public render(): React.ReactElement<IClockProps> {
    ...
    if(!this._isValidTimezone){
      return (
      <div className='invalidTimeZone'>
        Invalid Time Zone!
      </div>
      );
    }
    //valid timezone supplied
      ...
      this._dateString = Intl.DateTimeFormat('default', this._formatOptions).format(this.state.date);
      const hours : number = parseInt(Intl.DateTimeFormat('default', {hour: this._formatOptions.hour, timeZone: this._formatOptions.timeZone}).format(this.state.date),10);
      const minutes : number = parseInt(Intl.DateTimeFormat('default', {minute: this._formatOptions.minute, timeZone: this._formatOptions.timeZone}).format(this.state.date),10);
      const seconds : number = parseInt(Intl.DateTimeFormat('default', {second: this._formatOptions.second, timeZone: this._formatOptions.timeZone}).format(this.state.date),10);
  
      const hoursDegrees: number =    hours * 30 + minutes / 2;
      const minutesDegrees: number =  minutes * 6 + seconds / 10;
      const secondsDegrees: number =  seconds * 6;
  
      const divStyleHours: React.CSSProperties = {
        transform: 'rotateZ(' + hoursDegrees + 'deg)',
      };
  
      const divStyleMinutes: React.CSSProperties = {
        transform: 'rotateZ(' + minutesDegrees + 'deg)',
      };
  
      const divStyleSeconds: React.CSSProperties = {
        transform: 'rotateZ(' + secondsDegrees + 'deg)',
      };

      if (!this.props.digitalOnly) {
        return (
          <div className='clockContainer'> 
            <div className='analogContainer styling'>
              <div className='analogClock'>
                <svg
                  className='backgroundNumbers'
                ...
                <div
                  id='hours-indicator'
                  className='indicator hoursIndicator'
                  style={divStyleHours}
                ></div>
                <div
                  id='minutes-indicator'
                  className='indicator minutesIndicator'
                  style={divStyleMinutes}
                ></div>
                <div
                  id='seconds-indicator'
                  className='indicator secondsIndicator'
                  style={divStyleSeconds}
                ></div>
                <div className='indicatorCover'></div>
              </div>
            </div>
  
            <div className='digitalContainer'>
              {escape(this._dateString)}
            </div>
          </div>
        );
      } else {
