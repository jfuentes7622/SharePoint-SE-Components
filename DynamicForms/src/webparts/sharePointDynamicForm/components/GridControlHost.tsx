...
interface IGridControlHostState {
  status: 'waiting' | 'embedded' | 'missing';
}

export class GridControlHost extends React.Component<IGridControlHostProps, IGridControlHostState> {
  ...
  public constructor(props: IGridControlHostProps) {
    super(props);
  ...
  public componentDidMount(): void {
    if (typeof window !== 'undefined' && window.addEventListener) {
      window.addEventListener('spse:gridcontrol-runtime-config-request', this._runtimeConfigRequestHandler);
    }
    this.startDiscovery();
  }
  ...
  public componentDidUpdate(prevProps: IGridControlHostProps): void {
    ...
    if (this._targetHost && (
      prevProps.runtimeFilterJson !== this.props.runtimeFilterJson
      || prevProps.defaultField !== this.props.defaultField
      || prevProps.defaultValue !== this.props.defaultValue
      || prevProps.readOnly !== this.props.readOnly
    )) {
      this.dispatchRuntimeConfig(true);
    }
    if (this._targetHost && (prevProps.isPageEditMode !== this.props.isPageEditMode || prevProps.sourceListName !== this.props.sourceListName || prevProps.sourceName !== this.props.sourceName)) {
  ...
  public componentWillUnmount(): void {
    ...
    if (typeof window !== 'undefined' && window.removeEventListener) {
      window.removeEventListener('spse:gridcontrol-runtime-config-request', this._runtimeConfigRequestHandler);
    }
  ...
  private getInstanceId(sourceId?: string): string {
  ...
  private dispatchRuntimeConfig(active: boolean, sourceId?: string, ownerId?: string): void {
    ...
    if (typeof (window as any).CustomEvent === 'function') {
      runtimeEvent = new (window as any).CustomEvent('spse:gridcontrol-runtime-config', { detail: detail });
    } else {
      runtimeEvent = document.createEvent('CustomEvent');
      runtimeEvent.initCustomEvent('spse:gridcontrol-runtime-config', false, false, detail);
    }
  ...
  private startDiscovery(): void {
    this._attemptCount = 0;
    if (this.tryEmbedTarget()) { return; }
    this._retryHandle = window.setInterval(() => {
      this._attemptCount += 1;
      if (this.tryEmbedTarget()) { this.stopDiscovery(); }
      else if (this._attemptCount >= 40) { this.stopDiscovery(); this.setState({ status: 'missing' }); }
    }, 250);
  }

  private stopDiscovery(): void {
    if (this._retryHandle !== undefined) { window.clearInterval(this._retryHandle); this._retryHandle = undefined; }
  }
  ...
  private findTargetHost(): HTMLElement | null {
    ...
    for (var i = 0; i < roots.length; i += 1) {
      ...
      while (ancestor && ancestor !== document.body) {
        ...
        if (candidateId === instanceId || candidateId === sourceId || candidateId.indexOf(instanceId) >= 0) { return host; }
  ...
  private tryEmbedTarget(): boolean {
    if (!this.props.sourceId || !this._mountElement) { return false; }
    ...
    if (this.state.status !== 'embedded') { this.setState({ status: 'embedded' }); }
    this.dispatchRuntimeConfig(true);
    return true;
  }
  ...
  private restoreTarget(): void {
    ...
    this._targetHost.removeAttribute('data-dynamicforms-gridcontrol-owner');
    if (this._originalParent) {
      if (this._originalNextSibling && this._originalNextSibling.parentNode === this._originalParent) { this._originalParent.insertBefore(this._targetHost, this._originalNextSibling); }
      else { this._originalParent.appendChild(this._targetHost); }
    }
    this._targetHost = null;
    this._originalParent = null;
    this._originalNextSibling = null;
  ...
  private syncConfigurationMarker(): void {
    this.removeConfigurationMarker();
    if (!this.props.isPageEditMode || !this._originalParent) { return; }
    var marker = document.createElement('div');
    marker.setAttribute('data-dynamicforms-gridcontrol-configure', this.props.fieldId);
    ...
    marker.textContent = 'Configure Grid: ' + (this.props.sourceListName || this.props.sourceName || this.props.sourceId);
  ...
  public render(): JSX.Element {
    ...
    return (
      <div data-dynamicforms-gridcontrol-field={this.props.fieldId}>
        {this.state.status === 'waiting' && <div style={{ padding: '12px', color: '#605e5c' }}>Waiting for Grid Control "{sourceName}"...</div>}
        {this.state.status === 'missing' && <div style={{ padding: '12px', color: '#a80000' }}>Grid Control "{sourceName}" was not found on this page.</div>}
