import { BaseComponentContext, _IComponentContextParameters } from '@microsoft/sp-component-base';
import { GraphHttpClient, HttpClient, SPHttpClient } from '@microsoft/sp-http';
import { _IPreloadedData } from '@microsoft/sp-loader';
import { ISuiteNavManager } from './suiteNav/SuiteNavManager';
import Navigator from './navigator/Navigator';
/**
 * Constructor parameters for BaseApplicationContext.
 *
 * @internal
 */
export interface IApplicationContextParameters extends _IComponentContextParameters {
    suiteNavManager: ISuiteNavManager | undefined;
    navigator: Navigator;
}
/**
 * Context object for a client-side applications.
 *
 * @alpha
 */
export default class BaseApplicationContext extends BaseComponentContext {
    private _graphHttpClient;
    private _httpClient;
    private _spHttpClient;
    private _preloadedData;
    private _suiteNavManager;
    /** @internal */
    constructor(parameters: IApplicationContextParameters);
    /**
     * The instance of GraphHttpClient created for this application
     */
    readonly graphHttpClient: GraphHttpClient;
    /**
     * The instance of HttpClient created for this application
     */
    readonly httpClient: HttpClient;
    /**
     * The instance of SpHttpClient created for this application
     */
    readonly spHttpClient: SPHttpClient;
    /**
     * The preloaded data used by SPFx
     */
    readonly preloadedData: _IPreloadedData;
    /**
     * The SuiteNav manager
     *
     * @remarks
     * This can be undefined for chromeless applications. As this API is alpha it is OK to do so.
     * The experience for chromeless applications needs to be refactored.
     */
    readonly suiteNavManager: ISuiteNavManager | undefined;
}
