    ...
    while (index < expression.length) {
        ...
        if (character === '"' || character === '\'') {
            ...
            while (index < expression.length) {
                ...
                if (stringCharacter === '\\' && index + 1 < expression.length) {
                    stringValue += expression.charAt(index + 1);
                    index += 2;
                    continue;
                }
        ...
        if (/\d/.test(character)) {
            ...
            while (index < expression.length && /[\d.]/.test(expression.charAt(index))) {
                numberValue += expression.charAt(index);
                index += 1;
            }
        ...
        if (/[A-Za-z_]/.test(character)) {
            ...
            while (index < expression.length && /[A-Za-z0-9_]/.test(expression.charAt(index))) {
                identifier += expression.charAt(index);
                index += 1;
            }
...
function evaluateGridValidationExpression(expression, fields, values) {
    ...
    for (var fieldIndex = 0; fieldIndex < fields.length; fieldIndex += 1) {
        ...
        if (field.id) {
            fieldsByKey[field.id.toLowerCase()] = field;
        }
        if (field.fieldName) {
            fieldsByKey[field.fieldName.toLowerCase()] = field;
        }
        if (field.label) {
            fieldsByKey[field.label.toLowerCase()] = field;
        }
    }
    var peek = function () { return tokens[tokenIndex] || { type: 'eof', value: '' }; };
    var consume = function (expectedType, expectedValue) {
        var current = peek();
    ...
    var resolveFieldValue = function (fieldReference) {
    ...
    var parseUnary;
    var parsePrimary;
    var evaluateFunction = function (name, args) {
        ...
        if (functionName === 'daysfromtoday') {
            ...
            date.setDate(date.getDate() + (isNaN(delta) ? 0 : delta));
            return date;
        }
    ...
    parsePrimary = function () {
        ...
        if (token.type === 'paren' && token.value === '(') {
            consume('paren', '(');
            var parenthesized = parseExpression();
            consume('paren', ')');
...
exports.evaluateGridValidationExpression = evaluateGridValidationExpression;

//# sourceMappingURL=GridValidation.js.map
