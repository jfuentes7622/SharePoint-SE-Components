...
export function adjustOptionalFullWidthForScrollbar(documentRef: Document, ownerId: string): void { const ownedElements = documentRef.querySelectorAll('[' + OWNER_ATTRIBUTE + ']'); let scrollbarGutter = 0; for (let index = 0; index < ownedElements.length; index += 1) { const element = ownedElements[index] as HTMLElement; if (getOwners(element).indexOf(ownerId) < 0) { continue; } element.style.overflowY = element.getAttribute('data-sps-full-width-overflow-y') || ''; const computedStyle = documentRef.defaultView ? documentRef.defaultView.getComputedStyle(element) : undefined; const borderWidth = computedStyle ? parseFloat(computedStyle.borderLeftWidth || '0') + parseFloat(computedStyle.borderRightWidth || '0') : 0; scrollbarGutter = Math.max(scrollbarGutter, Math.max(0, element.offsetWidth - element.clientWidth - borderWidth)); } if (scrollbarGutter <= 0) { return; } for (let index = 0; index < ownedElements.length; index += 1) { const element = ownedElements[index] as HTMLElement; if (getOwners(element).indexOf(ownerId) < 0) { continue; } const appliedWidth = parseFloat(element.style.maxWidth || ''); if (!isNaN(appliedWidth) && appliedWidth > scrollbarGutter) { const adjustedWidth = String(Math.floor(appliedWidth - scrollbarGutter)) + 'px'; element.style.setProperty('min-width', adjustedWidth, 'important'); element.style.setProperty('max-width', adjustedWidth, 'important'); } } }
