...
const BREAKOUT_ATTRIBUTE = 'data-sps-full-width-breakout';
const ADDED_NATIVE_CLASS_ATTRIBUTE = 'data-sps-full-width-native-class';
const ORIGINAL_PREFIX = 'data-sps-full-width-';
...
function getBreakoutContainer(path: HTMLElement[]): HTMLElement | undefined {
	const preferredNames = ['CanvasZone', 'CanvasZoneContainer', 'CanvasZoneSectionContainer', 'CanvasSection'];
	for (let nameIndex = 0; nameIndex < preferredNames.length; nameIndex += 1) {
		for (let pathIndex = path.length - 1; pathIndex >= 0; pathIndex -= 1) {
			if (hasLayoutName(path[pathIndex], preferredNames[nameIndex])) { return path[pathIndex]; }
		}
	}
	return undefined;
}
...
export function updateResponsiveOptionalFullWidth(domElement: HTMLElement, ownerId: string, enabled: boolean): void {
	...
	const breakoutContainer = getBreakoutContainer(path);
	if (!breakoutContainer) { return; }
	const breakoutIndex = path.indexOf(breakoutContainer);
	path.slice(0, breakoutIndex + 1).forEach((element: HTMLElement) => addOwner(element, ownerId, element === breakoutContainer));
...
export function releaseOptionalFullWidth(documentRef: Document, ownerId: string): void {
	const monitor = RESPONSIVE_MONITORS[ownerId];
	if (monitor) {
		if (monitor.timer) { window.clearTimeout(monitor.timer); }
