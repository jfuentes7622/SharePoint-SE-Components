...
class Npm {
    static publishedVersions(packageName, cwd, env) {
        const versions = [];
        try {
            const packageTime = Utilities_1.default.executeCommandAndCaptureOutput('npm', `view ${packageName} time --json`.split(' '), cwd, env);
            if (packageTime && packageTime !== '') {
                ...
                const packageVersions = Utilities_1.default.executeCommandAndCaptureOutput('npm', `view ${packageName} versions --json`.split(' '), cwd, env);
