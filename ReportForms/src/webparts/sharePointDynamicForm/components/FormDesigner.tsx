...
export class FormDesigner extends React.Component<FormDesignerProps, FormDesignerState> {
  ...
  private getConditionalFieldLabel(fieldReference: string): string {
    ...
    var fields = this.getConditionalRuleFields();
    for (var i = 0; i < fields.length; i += 1) {
      if (String(fields[i].id || '').toLowerCase() === normalized || String(fields[i].fieldName || '').toLowerCase() === normalized) { return fields[i].label; }
    }
  ...
  private renderConditionalRules(): JSX.Element {
    ...
    var fieldOptions = fields.map(function(field) { return <option key={field.id} value={field.id}>{field.label + ' (' + field.fieldName + ')'}</option>; });
    ...
    return (
      <div className={styles.designerPanel}>
        <div style={{ display: 'flex', gap: '4px', paddingBottom: '12px', borderBottom: '1px solid #e1dfdd', marginBottom: '4px' }}>
          ...
          <button className={activeTab === 'conditional' ? styles.designerTabActive : styles.designerTab} onClick={() => this.setState({ activeDesignerTab: 'conditional' })}>Conditional styling</button>
        </div>
