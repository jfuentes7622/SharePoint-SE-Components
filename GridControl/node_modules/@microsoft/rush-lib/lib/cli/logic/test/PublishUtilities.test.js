...
describe('findChangeRequests', () => {
    ...
    beforeEach(() => {
        packagesRushConfiguration = RushConfiguration_1.default.loadFromConfigurationFile(path.resolve(__dirname, 'packages', 'rush.json'));
        repoRushConfiguration = RushConfiguration_1.default.loadFromConfigurationFile(path.resolve(__dirname, 'repo', 'rush.json'));
    });
    it('returns no changes in an empty change folder', () => {
        const allPackages = packagesRushConfiguration.projectsByName;
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'noChange')));
        chai_1.expect(Object.keys(allChanges).length).to.equal(0);
    });
    it('returns 1 change when changing a leaf package', () => {
        const allPackages = packagesRushConfiguration.projectsByName;
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'leafChange')));
    ...
    it('returns 2 changes when patching a root package', () => {
        const allPackages = packagesRushConfiguration.projectsByName;
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'rootPatchChange')));
    ...
    it('returns 4 changes when hotfixing a root package', () => {
        ...
        const allChanges = PublishUtilities_1.default.findChangeRequests(packagesRushConfiguration.projectsByName, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'rootHotfixChange')));
        chai_1.expect(Object.keys(allChanges).length).to.equal(4);
        chai_1.expect(allChanges).has.property('a');
        chai_1.expect(allChanges).has.property('b');
        chai_1.expect(allChanges['a'].changeType).equals(ChangeManagement_1.ChangeType.hotfix, 'a was not a hotfix');
        chai_1.expect(allChanges['b'].changeType).equals(ChangeManagement_1.ChangeType.hotfix, 'b did not receive a hotfix');
        chai_1.expect(allChanges['c'].changeType).equals(ChangeManagement_1.ChangeType.hotfix, 'c did not receive a hotfix');
        chai_1.expect(allChanges['d'].changeType).equals(ChangeManagement_1.ChangeType.hotfix, 'd did not receive a hotfix');
        chai_1.expect(allChanges['a'].newVersion).equals('1.0.0-hotfix.0', 'a was not hotfixed');
        chai_1.expect(allChanges['b'].newVersion).equals('1.0.0-hotfix.0', 'b did not receive a hotfix version bump');
        chai_1.expect(allChanges['c'].newVersion).equals('1.0.0-hotfix.0', 'c did not receive a hotfix version bump');
        chai_1.expect(allChanges['d'].newVersion).equals('1.0.0-hotfix.0', 'd did not receive a hotfix version bump');
    });
    it('returns 3 changes when major bumping a root package', () => {
        const allPackages = packagesRushConfiguration.projectsByName;
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'rootMajorChange')));
    ...
    it('returns 2 changes when bumping cyclic dependencies', () => {
        const allPackages = packagesRushConfiguration.projectsByName;
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'cyclicDeps')));
        chai_1.expect(Object.keys(allChanges).length).to.equal(2);
    ...
    it('returns error when mixing hotfix and non-hotfix changes', () => {
        const allPackages = packagesRushConfiguration.projectsByName;
        chai_1.expect(PublishUtilities_1.default.findChangeRequests.bind(PublishUtilities_1.default, allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'hotfixWithPatchChanges')))).to
    ...
    it('returns error when adding hotfix with config disabled', () => {
        ...
        chai_1.expect(PublishUtilities_1.default.findChangeRequests.bind(PublishUtilities_1.default, allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'rootHotfixChange')))).to
    ...
    it('can resolve multiple changes requests on the same package', () => {
        const allPackages = packagesRushConfiguration.projectsByName;
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'multipleChanges')));
        chai_1.expect(Object.keys(allChanges).length).to.equal(3);
        chai_1.expect(allChanges).has.property('a');
        chai_1.expect(allChanges).has.property('b');
        chai_1.expect(allChanges).has.property('c');
    ...
    it('can resolve multiple reverse-ordered changes requests on the same package', () => {
        const allPackages = packagesRushConfiguration.projectsByName;
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'orderedChanges')));
        chai_1.expect(Object.keys(allChanges).length).to.equal(3);
        chai_1.expect(allChanges).has.property('a');
        chai_1.expect(allChanges).has.property('b');
        chai_1.expect(allChanges).has.property('c');
    ...
    it('can resolve multiple hotfix changes', () => {
        const allPackages = packagesRushConfiguration.projectsByName;
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'multipleHotfixChanges')));
        chai_1.expect(Object.keys(allChanges).length).to.equal(4);
        chai_1.expect(allChanges).has.property('a');
        chai_1.expect(allChanges).has.property('b');
        chai_1.expect(allChanges).has.property('c');
    ...
    it('can update an explicit dependency', () => {
        const allPackages = packagesRushConfiguration.projectsByName;
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, packagesRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'explicitVersionChange')));
        chai_1.expect(Object.keys(allChanges).length).to.equal(2);
        chai_1.expect(allChanges).has.property('c');
        chai_1.expect(allChanges).has.property('d');
    ...
    it('can exclude lock step projects', () => {
        ...
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, repoRushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'repo', 'changes')), false, undefined, new Set(['a', 'b', 'e']));
...
describe('sortChangeRequests', () => {
    ...
    beforeEach(() => {
        rushConfiguration = RushConfiguration_1.default.loadFromConfigurationFile(path.resolve(__dirname, 'packages', 'rush.json'));
    });
    it('can return a sorted array of the change requests to be published in the correct order', () => {
        ...
        const allChanges = PublishUtilities_1.default.findChangeRequests(allPackages, rushConfiguration, new ChangeFiles_1.default(path.join(__dirname, 'multipleChanges')));
...
describe('getNewDependencyVersion', () => {
    ...
    it('can update to prerelease', () => {
        ...
        chai_1.expect(PublishUtilities_1.default.getNewDependencyVersion(dependencies, 'a', '1.0.0-hotfix.0')).equals('~1.0.0-hotfix.0');
        chai_1.expect(PublishUtilities_1.default.getNewDependencyVersion(dependencies, 'b', '1.0.0-hotfix.0')).equals('^1.0.0-hotfix.0');
        chai_1.expect(PublishUtilities_1.default.getNewDependencyVersion(dependencies, 'c', '1.0.0-hotfix.0')).equals('>=1.0.0-hotfix.0 <2.0.0');
    });
});
