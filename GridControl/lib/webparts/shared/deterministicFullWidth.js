...
function saveStyle(element, propertyName) { element.setAttribute(ORIGINAL_PREFIX + propertyName, element.style.getPropertyValue(propertyName) || ''); element.setAttribute(ORIGINAL_PREFIX + propertyName + '-priority', element.style.getPropertyPriority(propertyName) || ''); }
function restoreStyle(element, propertyName) { var value = element.getAttribute(ORIGINAL_PREFIX + propertyName) || ''; var priority = element.getAttribute(ORIGINAL_PREFIX + propertyName + '-priority') || ''; element.style.setProperty(propertyName, value, priority); element.removeAttribute(ORIGINAL_PREFIX + propertyName); element.removeAttribute(ORIGINAL_PREFIX + propertyName + '-priority'); }
function addOwner(element, ownerId, isBreakout) { var owners = getOwners(element); if (owners.length === 0) {
...
} element.style.setProperty('width', '100%', 'important'); element.style.setProperty('min-width', '0', 'important'); element.style.setProperty('max-width', 'none', 'important'); element.style.setProperty('flex-basis', '100%', 'important'); element.style.setProperty('flex-grow', '1', 'important'); element.style.setProperty('flex-shrink', '1', 'important'); element.style.setProperty('grid-column', '1 / -1', 'important'); element.style.setProperty('box-sizing', 'border-box', 'important'); if (isBreakout) {
...
function removeOwner(element, ownerId) { var owners = getOwners(element).filter(function (owner) { return owner !== ownerId; }); if (owners.length > 0) {
    ...
    return;
} ['width', 'min-width', 'max-width', 'flex-basis', 'flex-grow', 'flex-shrink', 'grid-column', 'box-sizing', 'margin-left', 'margin-right', 'overflow-x', 'overflow-y'].forEach(function (propertyName) { return restoreStyle(element, propertyName); }); if (element.getAttribute(ADDED_NATIVE_CLASS_ATTRIBUTE) === 'true') {
    element.classList.remove('CanvasZone--fullWidth');
} element.removeAttribute(OWNER_ATTRIBUTE); element.removeAttribute(BREAKOUT_ATTRIBUTE); element.removeAttribute(ADDED_NATIVE_CLASS_ATTRIBUTE); }
function getVisiblePropertyPaneLeft(documentRef, viewportWidth) { var pane = documentRef.querySelector('#spPropertyPaneContainer, .spPropertyPaneContainer, [data-automation-id="propertyPane"]'); if (!pane) {
    return 0;
} var paneRect = pane.getBoundingClientRect(); return paneRect.width > 0 && paneRect.left > 0 && paneRect.left < viewportWidth ? paneRect.left : 0; }
function getBreakoutContainer(path) { var preferredNames = ['CanvasZone', 'CanvasZoneContainer', 'CanvasZoneSectionContainer', 'CanvasSection']; for (var nameIndex = 0; nameIndex < preferredNames.length; nameIndex += 1) {
...
function applyAvailableWidth(domElement, ownerId) { var documentRef = domElement.ownerDocument; var viewportWidth = documentRef.documentElement.clientWidth; var paneLeft = getVisiblePropertyPaneLeft(documentRef, viewportWidth); var hostname = String(documentRef.location && documentRef.location.hostname || '').toLowerCase(); var isLocal = hostname === 'localhost' || hostname === '127.0.0.1'; var isSharePointOnline = /\.sharepoint(?:-mil)?\.[a-z.]+$/.test(hostname); var rightGutter = !isLocal && !isSharePointOnline ? 20 : 0; var availableRight = paneLeft > 0 ? paneLeft : viewportWidth; var ownedElements = documentRef.querySelectorAll('[' + OWNER_ATTRIBUTE + ']'); for (var index = 0; index < ownedElements.length; index += 1) {
...
function scheduleUpdate(domElement, ownerId) { var monitor = RESPONSIVE_MONITORS[ownerId]; if (!monitor) {
    return;
} if (monitor.timer) {
...
function updateResponsiveOptionalFullWidth(domElement, ownerId, enabled) { releaseOptionalFullWidth(domElement.ownerDocument, ownerId); if (!enabled) {
    return;
} var path = []; var ancestor = domElement.parentElement; var depth = 0; while (ancestor && ancestor !== domElement.ownerDocument.body && depth < 32) {
    path.push(ancestor);
    ancestor = ancestor.parentElement;
    depth += 1;
} var breakoutContainer = getBreakoutContainer(path); if (!breakoutContainer) {
    return;
} var breakoutIndex = path.indexOf(breakoutContainer); path.slice(0, breakoutIndex + 1).forEach(function (element) { return addOwner(element, ownerId, element === breakoutContainer); }); applyAvailableWidth(domElement, ownerId); var documentRef = domElement.ownerDocument; var MutationObserverConstructor = documentRef.defaultView && documentRef.defaultView.MutationObserver; var monitor = { observer: undefined, timer: 0, resizeHandler: function () { return scheduleUpdate(domElement, ownerId); } }; if (MutationObserverConstructor && documentRef.body) {
