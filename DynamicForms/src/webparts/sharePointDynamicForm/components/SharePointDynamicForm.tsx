...
type FilterConditionOperator = 'eq' | 'ne' | 'gt' | 'ge' | 'lt' | 'le' | 'contains' | 'startswith' | 'endswith';
...
function normalizeFilterOperator(value: any): FilterConditionOperator {
  ...
  if (op === 'ne' || op === 'gt' || op === 'ge' || op === 'lt' || op === 'le' || op === 'contains' || op === 'startswith' || op === 'endswith') {
...
function buildODataFilterClause(fieldName: string, fieldType: string, value: any, operator?: FilterConditionOperator): string {
  ...
  switch (fieldType) {
    ...
    case 'person': {
      ...
      if (isNaN(numVal)) { 
        console.warn('[SharePointDynamicForm] Invalid lookup value for field ' + fieldName + ': ' + value);
        return ''; 
      }
      clause = fieldName + 'Id ' + op + ' ' + numVal;
      console.log('[SharePointDynamicForm] Built lookup filter: ' + clause + ' (field=' + fieldName + ', value=' + value + ')');
      return clause;
    }
    case 'number': {
      if (op === 'contains' || op === 'startswith' || op === 'endswith') {
        console.warn('[SharePointDynamicForm] Unsupported number operator for field ' + fieldName + ': ' + op + '. Falling back to eq.');
        op = 'eq';
      }
      var n = Number(value);
      if (isNaN(n)) { 
        console.warn('[SharePointDynamicForm] Invalid number value for field ' + fieldName + ': ' + value);
        return ''; 
      }
      clause = fieldName + ' ' + op + ' ' + n;
      console.log('[SharePointDynamicForm] Built number filter: ' + clause);
      return clause;
    }
    case 'boolean': {
      var bv = value === true || value === 'true' || value === '1';
      if (op !== 'eq' && op !== 'ne') {
        console.warn('[SharePointDynamicForm] Unsupported boolean operator for field ' + fieldName + ': ' + op + '. Falling back to eq.');
        op = 'eq';
      }
      clause = fieldName + ' ' + op + ' ' + (bv ? 'true' : 'false');
      console.log('[SharePointDynamicForm] Built boolean filter: ' + clause);
      return clause;
    }
    case 'datetime': {
      if (op === 'contains' || op === 'startswith' || op === 'endswith') {
    ...
    default: // choice, text, multiline, etc.
      if (op === 'contains' || op === 'startswith' || op === 'endswith') {
        clause = op + '(' + fieldName + ", '" + escapeODataText(String(value)) + "')";
      } else {
        clause = fieldName + ' ' + op + " '" + escapeODataText(String(value)) + "'";
      }
      console.log('[SharePointDynamicForm] Built text/choice filter: ' + clause + ' (type=' + fieldType + ')');
      return clause;
  }
}
...
function parseFilterJsonToExpression(filterJson: string, dynamicValue: any, fields: FormField[], context: any): string {
  ...
  try {
    ...
    if (!Array.isArray(conditions)) { 
      console.warn('[SharePointDynamicForm] filterJson is not an array: ' + filterJson);
      return expression; 
    }
    console.log('[SharePointDynamicForm] Parsing filterJson with ' + conditions.length + ' conditions');
    
    for (var i = 0; i < conditions.length; i++) {
      var cond = conditions[i];
      if (!cond || !cond.field) { 
        console.warn('[SharePointDynamicForm] Skipping invalid condition at index ' + i + ': missing field');
        continue; 
      }
      ...
      console.log('[SharePointDynamicForm] Condition ' + i + ': field=' + cond.field + ', type=' + (cond.type || '(auto)') + ', value=' + val);
      ...
      if (!condType) {
        for (var j = 0; j < fields.length; j++) {
          if (fields[j].fieldName && fields[j].fieldName.toLowerCase() === String(cond.field).toLowerCase()) {
            ...
            console.log('[SharePointDynamicForm] Resolved type from schema: ' + condType);
            break;
          }
        }
      }
      ...
      if (clause) {
        ...
        console.log('[SharePointDynamicForm] Added filter clause: ' + clause);
      } else {
        console.warn('[SharePointDynamicForm] Failed to build clause for condition ' + i);
      }
    }
  } catch (parseError) {
    console.error('[SharePointDynamicForm] Error parsing filterJson: ', parseError);
    if (parseError && parseError.message) {
      console.error('[SharePointDynamicForm] Parse error message: ' + parseError.message);
    }
  }
  return expression;
}
...
export class SharePointDynamicFormContainer extends React.Component<SharePointDynamicFormContainerProps, SharePointDynamicFormContainerState> {
  ...
  public componentDidMount(): void {
    ...
    console.log('[SharePointDynamicForm] Component mounted. Props:', {
  ...
  public componentDidUpdate(prevProps: SharePointDynamicFormContainerProps): void {
    ...
    if (requiresFullReload) {
      console.log('[SharePointDynamicForm] Props changed, reloading data');
      this.loadData();
      return;
    }
  ...
  private getResolvedItemId(schema?: FormSchema | null): number {
    ...
    if (pageContext && pageContext.listItem && pageContext.listItem.id) {
  ...
  private logDiagnostic(message: string): void {
    if (this.props.enableDynamicDiagnostics !== false) {
      console.log('[SharePointDynamicForm] ' + message);
    }
  }
  ...
  private logDynamicDiagnosticsToConsole(summary: string): void {
    ...
    console.log('[SharePointDynamicForm] ' + summary);
    for (var i = 0; i < entries.length; i += 1) {
      console.log('[SharePointDynamicForm] Dynamic diagnostics: ' + entries[i]);
    }
  }
  ...
  private extractPermissionFlags(raw: any): { canAdd: boolean; canEdit: boolean } {
    ...
    return {
      ...
      canEdit: effectivePermissions.hasPermission(SPPermission.editListItems),
    };
  }
  ...
  private renderDescriptionIcon(description: string): JSX.Element | null {
    ...
    console.log('[SharePointDynamicForm] renderDescriptionIcon - Rendering icon for description: "' + description + '"');
    return (
      <span 
        title={description}
  ...
  private renderFieldHelpAndError(description: string, errorMessage: string, labelPosition?: string): JSX.Element | null {
    ...
    console.log('[SharePointDynamicForm] renderFieldHelpAndError - description: "' + description + '", errorMessage: "' + errorMessage + '"');
  ...
  private async loadData(): Promise<void> {
    ...
    try {
      ...
      var resolvedItemId = this.getResolvedItemId(schema);
      console.log('[SharePointDynamicForm] Initial resolvedItemId from getResolvedItemId: ' + resolvedItemId);
      
      // If no item ID resolved yet, try to find a matching record via filter
      if (resolvedItemId <= 0) {
        console.log('[SharePointDynamicForm] No item ID resolved, attempting filter-based resolution...');
        resolvedItemId = await this.resolveFilteredItemId(schema);
        console.log('[SharePointDynamicForm] Filter resolution returned: ' + resolvedItemId);
      }
      var fields = getAllFields(schema);
      var webPartDefaults = parseDefaultValuesJson(this.props.defaultValuesJson);

      console.log('[SharePointDynamicForm] Building field values and loading lookup options...');
      console.log('[SharePointDynamicForm] Total fields in schema: ' + fields.length);

      for (var i = 0; i < fields.length; i += 1) {
        var field = fields[i];
        console.log('[SharePointDynamicForm] Field: ' + field.fieldName + ' (ID: ' + field.id + ', Type: ' + field.type + ')');
        ...
        if (field.type === 'lookup') {
          console.log('[SharePointDynamicForm] LOOKUP FIELD FOUND: ' + field.fieldName);
          console.log('[SharePointDynamicForm]   - field.config exists? ' + (!!field.config));
          if (field.config) {
            console.log('[SharePointDynamicForm]   - field.config.lookupList: ' + (field.config.lookupList || '(NOT SET)'));
            console.log('[SharePointDynamicForm]   - field.config.lookupField: ' + (field.config.lookupField || '(NOT SET, will default to "Title")'));
          } else {
            console.log('[SharePointDynamicForm]   - ERROR: field.config is NULL! Lookup options will not load!');
          }
          
          if (field.config && field.config.lookupList) {
            console.log('[SharePointDynamicForm] ✓ LOADING lookup options for field: ' + field.fieldName + ' from list: ' + field.config.lookupList);
            try {
              lookupOptions[field.id] = await this.loadLookupOptions(field.config.lookupList, field.config.lookupField || 'Title');
              console.log('[SharePointDynamicForm] ✓ Loaded ' + lookupOptions[field.id].length + ' lookup options for field: ' + field.fieldName);
            } catch (lookupError) {
              console.error('[SharePointDynamicForm] ✗ ERROR loading lookup options for ' + field.fieldName + ': ', lookupError);
              lookupOptions[field.id] = [];
            }
          } else {
            console.log('[SharePointDynamicForm] ✗ SKIPPING lookup load: field.config or lookupList not set for ' + field.fieldName);
            lookupOptions[field.id] = [];
          }
        } else if (field.type === 'person') {
          try {
            lookupOptions[field.id] = await this.loadPersonOptions();
            console.log('[SharePointDynamicForm] ✓ Loaded ' + lookupOptions[field.id].length + ' person options for field: ' + field.fieldName);
          } catch (personError) {
            console.error('[SharePointDynamicForm] ✗ ERROR loading person options for ' + field.fieldName + ': ', personError);
            lookupOptions[field.id] = [];
          }
        }
      }
      ...
      if (resolvedItemId > 0 && (this.props.mode === 'edit' || this.props.mode === 'view')) {
        loadedItem = await this.loadItem(schema.listName || this.props.listName, resolvedItemId);
        if (loadedItem) {
      ...
      var permissionFailureMessage = 'Permission evaluation failed. Disabling Add/Edit. Details: ' + (error && error.message ? error.message : strings.CommonUnknownError);
  ...
  private async resolveFilteredItemId(schema: FormSchema): Promise<number> {
    ...
    console.log('[SharePointDynamicForm] FILTER RESOLUTION STARTING. useDynamicValueAsFilter=' + this.props.useDynamicValueAsFilter + ', linkedFieldTarget=' + (this.props.linkedFieldTarget || '(none)') + ', linkedFieldValue=' + this.props.linkedFieldValue + ', urlDynamicValue=' + (urlDynamicValue || '(none)') + ', filterJson=' + (this.props.filterJson ? '(set)' : '(none)'));
    ...
    if (this.props.useDynamicValueAsFilter && this.props.linkedFieldTarget && hasUsableValue(preferredDynamicValue)) {
      console.log('[SharePointDynamicForm] FILTER ENABLED: Building dynamic filter from linkedFieldTarget');
      var targetLower = (this.props.linkedFieldTarget || '').trim().toLowerCase();
      console.log('[SharePointDynamicForm] Looking for field matching target (case-insensitive): "' + targetLower + '"');
      for (var i = 0; i < fields.length; i++) {
        ...
        console.log('[SharePointDynamicForm]   Checking field: id=' + (f.id || '(none)') + ', fieldName=' + (f.fieldName || '(none)') + ', idMatch=' + fieldIdMatch + ', nameMatch=' + fieldNameMatch);
        if (fieldIdMatch || fieldNameMatch) {
          console.log('[SharePointDynamicForm] FOUND MATCHING FIELD: ' + f.fieldName + ' (type=' + f.type + ')');
          var clause = buildODataFilterClause(f.fieldName, f.type, preferredDynamicValue);
          if (clause) { 
            filterExpressions.push(clause);
            console.log('[SharePointDynamicForm] Built filter clause: ' + clause);
          } else {
            console.warn('[SharePointDynamicForm] FAILED to build filter clause for field: ' + f.fieldName);
          }
          break;
        }
      }
      ...
      if (!this.props.useDynamicValueAsFilter) {
        console.log('[SharePointDynamicForm] FILTER DISABLED: useDynamicValueAsFilter is FALSE. Enable the toggle to filter by linked field value.');
      } else if (!this.props.linkedFieldTarget) {
        console.log('[SharePointDynamicForm] FILTER SKIPPED: linkedFieldTarget is not set');
      } else {
        console.log('[SharePointDynamicForm] FILTER SKIPPED: dynamic filter value is not usable. Value: ' + preferredDynamicValue);
      }
    }

    // Static filter from filterJson property
    if (this.props.filterJson) {
      console.log('[SharePointDynamicForm] STATIC FILTER: Processing filterJson');
      ...
      console.log('[SharePointDynamicForm] Static filter expression=' + (staticExpression || '(empty)') + '. Total expressions: ' + filterExpressions.length);
    }

    if (filterExpressions.length === 0) { 
      console.log('[SharePointDynamicForm] NO FILTER CLAUSES. Returning 0 (no record to load).');
      return 0; 
    }
    ...
    console.log('[SharePointDynamicForm] EXECUTING FILTER. List: ' + listName + ', Filter: ' + filterStr);

    var endpoint = this.getWebUrl()
      + "/_api/web/lists/getByTitle('" + escapeODataText(listName) + "')/items?$filter="
      + encodeURIComponent(filterStr) + '&$select=Id&$top=1';

    console.log('[SharePointDynamicForm] REST CALL: ' + endpoint);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error('[SharePointDynamicForm] Response body: ' + errorText);
        } catch (_textError) {
  ...
  private async loadLookupOptions(lookupList: string, lookupField: string): Promise<ILookupOptionItem[]> {
    console.log('[SharePointDynamicForm] loadLookupOptions START - List: ' + lookupList + ', Field: ' + lookupField);
    ...
    console.log('[SharePointDynamicForm] Is GUID? ' + isGuid(lookupList) + ', Clean List: ' + cleanLookupList);
    console.log('[SharePointDynamicForm] REST Endpoint: ' + endpoint);
    
    try {
      var response = await this.getWithAcceptFallback(endpoint);
      
      console.log('[SharePointDynamicForm] REST Response Status: ' + response.status + ' (' + response.statusText + ')');
      
      if (!response.ok) {
        console.error('[SharePointDynamicForm] REST call failed! Status: ' + response.status);
        return [];
      }
      ...
      console.log('[SharePointDynamicForm] loadLookupOptions SUCCESS - Returning ' + mappedItems.length + ' items');
      return mappedItems;
    } catch (error) {
      console.error('[SharePointDynamicForm] loadLookupOptions ERROR: ', error);
      return [];
    }
  }
  ...
  private async loadFieldDescriptions(schema: FormSchema): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn('[SharePointDynamicForm] loadFieldDescriptions: Failed to fetch fields, status ' + response.status);
        return;
      }
      ...
      if (!fields || !Array.isArray(fields)) {
        console.warn('[SharePointDynamicForm] loadFieldDescriptions: Invalid response format');
        return;
      }
      ...
      fields.forEach((field: any) => {
        if (field.InternalName) {
          ...
          if (field.Description) {
            console.log('[SharePointDynamicForm] Found description for "' + field.InternalName + '": "' + field.Description + '"');
          }
        }
      });
  ...
  this._listFieldTypeLookup = fieldTypeLookup;
        ...
        if (step.fields) {
          step.fields.forEach((field) => {
            ...
            if (fieldMeta.description && !field.description) {
              ...
              console.log('[SharePointDynamicForm] Populated description for field "' + field.fieldName + '": "' + field.description + '"');
            }
            ...
            if (field.type === 'multiline' && fieldMeta.richText === true) {
              ...
              if (field.config.isRichText !== true) {
                ...
                console.log('[SharePointDynamicForm] Marked field "' + field.fieldName + '" as rich text note field');
              }
            }
          });
        }
      ...
      console.log('[SharePointDynamicForm] loadFieldDescriptions SUCCESS');
    } catch (error) {
      this._listFieldNameLookup = {};
      this._listFieldInternalNameLookup = {};
      this._listFieldTypeLookup = {};
  ...
  private buildSubmitPayload(schema: FormSchema): any {
    ...
    if (this.props.useDynamicValueAsFilter && linkedTargetInternalName && linkedLookupId
      ...
      console.log('[SharePointDynamicForm] Applied dynamic parent lookup payload: ' + linkedTargetInternalName + 'Id=' + linkedLookupId);
    }
    return payload;
  }
    ...
    switch (field.type) {
      ...
      case 'lookup':
        ...
        if (lookupValue || lookupItems.length > 0) {
          ...
          console.log('[SharePointDynamicForm] ===== LOOKUP FIELD DIAGNOSTIC =====');
          console.log('[SharePointDynamicForm] Field: ' + field.label + ' (' + field.fieldName + ', ID: ' + field.id + ')');
          console.log('[SharePointDynamicForm] Current Value: "' + lookupValue + '" (type: ' + typeof lookupValue + ', length: ' + String(lookupValue).length + ')');
          console.log('[SharePointDynamicForm] Available Options (' + lookupOptions.length + '): [' + optionKeysList + ']');
          console.log('[SharePointDynamicForm] Selected Option: ' + (foundOption ? '"' + foundOption.text + '"' : 'NOT FOUND - VALUE MISMATCH!'));
          console.log('[SharePointDynamicForm] =====================================');
        }
        return (
          <div style={fieldWrapperStyle}>
            <div style={Object.assign({ marginBottom: '10px' }, labelStyle)}>{field.label}{this.renderDescriptionIcon(description)}</div>
            ...
            {existingAttachments.length > 0 && (
              <div style={{ marginBottom: '10px' }}>
                {existingAttachments.map((attachment) => {
                  ...
                  return (
                    <div key={attachment.fileName} style={{ marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <span style={markedForDelete ? { textDecoration: 'line-through', color: '#a80000' } : undefined}>
                      ...
                      {markedForDelete && <span style={{ fontSize: '12px', color: '#a80000' }}>{strings.FieldAttachmentWillDelete}</span>}
  ...
  public render(): JSX.Element {
    ...
    // Font settings for form wrapper: always resolved (own default when unset) so a form never
    // silently inherits font styling meant for the web part chrome around it.
    containerStyle.fontSize = (schema.theme && schema.theme.fontSize) || 14;
    containerStyle.fontFamily = (schema.theme && schema.theme.fontFamily) || DEFAULT_THEME_FONT_FAMILY;
    containerStyle.fontWeight = ((schema.theme && schema.theme.fontWeight) || 'normal') as any;
    containerStyle.color = (schema.theme && schema.theme.color) || '#000000';
    if (schema.theme) {
    ...
    return (
      <div className={styles.sharePointDynamicForm + ' ' + styles.container + (this.props.isDarkTheme ? ' ' + styles.dark : '')} style={containerStyle}>
        ...
        {(() => {
          ...
          // Apply step theme styling. Font settings always resolve to their own default when unset
          // so a container never inherits form-level font styling meant for the surrounding form.
          ...
          return (
          <div key={step.id}>
            {step.showTitle !== false && step.title && <h2 style={{
            ...
            {step.showTitle !== false && step.description && <p style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', marginTop: '24px' }}>
            ...
            <PrimaryButton
              styles={primaryButtonStyles as any}
              onClick={() => this.setState({ isViewEditing: true, submitError: null, submitSuccess: null, fieldErrors: {} })}
